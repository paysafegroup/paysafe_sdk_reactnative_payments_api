...
- **@paysafe/paysafe-card-payments** v2.0.1
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v2.0.2
- **@paysafe/paysafe-payments-sdk-common** v2.0.0
- **@paysafe/react-native-paysafe-apple-pay** v2.0.2

### Fixed

- **@paysafe/paysafe-venmo** v2.0.2 — Removed unnecessary `as NSString` / `as NSURL` casts from Swift AppDelegate examples and the Expo config plugin. The C API (`PaysafeVenmoConfigureAppSwitchReturnURLScheme`, `PaysafeVenmoHandleAppSwitchOpenURL`) accepts Swift `String` and `URL` values directly, matching the Objective‑C++ integration pattern.
...
- **@paysafe/paysafe-card-payments** v2.0.1
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v2.0.1
- **@paysafe/paysafe-payments-sdk-common** v2.0.0
- **@paysafe/react-native-paysafe-apple-pay** v2.0.2

### Changed

- **@paysafe/paysafe-venmo** v2.0.1 — Migrated `PaysafeVenmo` to a Turbo Module (codegen spec `PaysafeVenmoSpec`, `RCTNativePaysafeVenmo` on iOS, `BaseReactPackage` on Android). Requires React Native **0.76+**, the New Architecture, and **@paysafe/paysafe-payments-sdk-common** v2.0.0+. **Breaking:** `initializeVenmo` and `tokenizeVenmo` now return `Promise` values instead of void methods completed via `DeviceEventEmitter` (`VenmoInitializedSuccessful`, `VenmoTokenizationSuccessful`, etc.). Rejections use codes `VENMO_INITIALIZATION_FAILED`, `VENMO_TOKENIZATION_FAILED`, and `VENMO_TOKENIZATION_CANCELED`. Adds an Expo config plugin for Venmo app-switch `AppDelegate` wiring (Expo SDK 53+). Apps on the legacy bridge should stay on **1.x**. Released as **v2.0.1** because **v2.0.0** was previously published and unpublished on npm and cannot be republished.
...
- **@paysafe/paysafe-card-payments** v2.0.1
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v1.5.0
- **@paysafe/paysafe-payments-sdk-common** v2.0.0
- **@paysafe/react-native-paysafe-apple-pay** v2.0.2

### Fixed

- **@paysafe/paysafe-venmo** v1.5.0 — Fixed Android Venmo initialization and tokenization. Native init now runs through a headless `VenmoInitFragment` so the Paysafe SDK can register its `ActivityResultLauncher` before the activity reaches STARTED (required for React Native activities that are already resumed). Tokenization uses the foreground activity context when available. Public JS API (`initializeVenmo`, `tokenizeVenmo`, and device events) is unchanged.
...
- **@paysafe/paysafe-card-payments** v2.0.1
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v1.4.0
- **@paysafe/paysafe-payments-sdk-common** v2.0.0
- **@paysafe/react-native-paysafe-apple-pay** v2.0.2

### Changed

- **@paysafe/paysafe-card-payments** v2.0.1 — Migrated `PaysafeCardPayments` to a Turbo Module (codegen spec `PaysafeCardPaymentsSpec`, `RCTNativePaysafeCardPayments` on iOS, `BaseReactPackage` on Android). Requires React Native **0.76+**, the New Architecture, and **@paysafe/paysafe-payments-sdk-common** v2.0.0+. Public JS API (`initialize`, `tokenize`, `setupPaysafeSdk`, hosted card field views, and device events) is unchanged. Apps on the legacy bridge should stay on **1.x**. Released as **v2.0.1** because **v2.0.0** was previously published and unpublished on npm and cannot be republished.
...
- **@paysafe/paysafe-card-payments** v1.3.0
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v1.4.0
- **@paysafe/paysafe-payments-sdk-common** v2.0.0
- **@paysafe/react-native-paysafe-apple-pay** v2.0.2
...
- **@paysafe/react-native-paysafe-apple-pay** v2.0.2 — Fixed Android crash (`TurboModuleRegistry.getEnforcing(...): 'RNPaysafeApplePay' could not be found`) when the package is imported on Android. Native module loading is now lazy via `TurboModuleRegistry.get()` and only resolves on iOS; public JS API is unchanged.

### Changed

- **@paysafe/paysafe-venmo** v1.4.0 — Requires **@paysafe/paysafe-payments-sdk-common** v2.0.0+ as a peer dependency (aligns with the Turbo Module migration in the common SDK). Public JS API is unchanged.
...
- **@paysafe/paysafe-card-payments** v1.3.1

### Fixed

- **`@paysafe/paysafe-card-payments`** — Fixed Android card form fields (card number, CVV, expiry date, cardholder name) not rendering correctly by deferring Compose view attachment from `init` to `onAttachedToWindow()`, preventing view lifecycle and "already has a parent" crashes.
...
- **@paysafe/paysafe-card-payments** v1.3.0
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v1.3.1
- **@paysafe/paysafe-payments-sdk-common** v2.0.0
- **@paysafe/react-native-paysafe-apple-pay** v2.0.0

### Changed

- **@paysafe/paysafe-payments-sdk-common** v2.0.0 — Migrated `PaysafeSDK` to a Turbo Module (codegen spec `PaysafeSDKSpec`, `BaseReactPackage` on Android, `RCTNativePaysafeSDK` on iOS). Requires React Native **0.76+** and the New Architecture. Public JS API (`setup`, `isInitialized`, `getMerchantReferenceNumber`) is unchanged. Apps on the legacy bridge should stay on **1.x**.
- **@paysafe/react-native-paysafe-apple-pay** v2.0.0 — Migrated `RNPaysafeApplePay` to a Turbo Module (codegen spec `RNPaysafeApplePaySpec`, `RCTNativeRNPaysafeApplePay` on iOS). Requires React Native **0.76+** and the New Architecture, and **@paysafe/paysafe-payments-sdk-common** v2.0.0+. Public JS API (`initializeApplePayContext`, `tokenize`, `resetApplePayContext`, `isApplePayAvailable`) is unchanged. Apps on the legacy bridge should stay on **1.x**.
...
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v1.3.1
- **@paysafe/paysafe-payments-sdk-common** v1.0.4
- **@paysafe/react-native-paysafe-apple-pay** v1.0.1

### Added

- **`@paysafe/paysafe-card-payments` v1.3.0** — iOS card payments implementation: native module and hosted field view managers, tokenize options parsing (`PSCardRNTokenizeOptionsParser`), expanded TypeScript types, and peer dependency on `@paysafe/paysafe-payments-sdk-common`. Android native module surface aligned with the shared tokenize options model.
- **DemoAppExpo** — Dedicated card payments demo screen (`cardPaymentsScreen`), shared demo hook and env/config helpers, and Jest coverage updates for card flows.

### Fixed

- **`@paysafe/paysafe-card-payments` v1.3.0** — Include `PaysafeCardPayments.podspec` in the published npm package so CocoaPods autolinking works for registry installs. Add `prepublishOnly` to build `dist/` before npm publish.
- **`@paysafe/paysafe-venmo` v1.3.1** — Include `PaysafeVenmo.podspec` in the published npm package so CocoaPods autolinking works for registry installs. Add `prepublishOnly` to build `dist/` before npm publish.
...
- **@paysafe/paysafe-card-payments** v1.2.2
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v1.3.0
- **@paysafe/paysafe-payments-sdk-common** v1.0.4
- **@paysafe/react-native-paysafe-apple-pay** v1.0.1

### Added

- **`@paysafe/paysafe-venmo` v1.3.0** — iOS helpers for AppDelegate URL handling and Venmo return URLs; Android tokenize options parsing and native module updates; expanded TypeScript surface and tests.
- **DemoAppExpo** — Venmo demo flow, Expo Router native intent / deep-link handling for return URLs, and Jest coverage for the demo and native modules.
...
- **@paysafe/paysafe-card-payments** v1.2.2
- **@paysafe/paysafe-google-pay** v1.2.2
- **@paysafe/paysafe-venmo** v1.2.2
- **@paysafe/paysafe-payments-sdk-common** v1.0.3
- **@paysafe/react-native-paysafe-apple-pay** v1.0.1

### Fixed

- **@paysafe/react-native-paysafe-apple-pay** — Added `tslib` as a dependency to fix runtime issues in consuming apps.
...
- **@paysafe/paysafe-venmo** v1.2.2
- **@paysafe/paysafe-payments-sdk-common** v1.0.3
- **@paysafe/react-native-paysafe-apple-pay** v1.0.0 (published to npm)

### Added

- **Apple Pay for React Native (`@paysafe/react-native-paysafe-apple-pay`)** — npm package name `@paysafe/react-native-paysafe-apple-pay`, first stable release on the npm registry. iOS native module (`RNPaysafeApplePay`) for Paysafe Apple Pay: initialize context, reset, tokenize, and PassKit availability. TypeScript API with client-side validation; CocoaPods integration via `react-native-paysafe-apple-pay.podspec`.
...
- **@paysafe/paysafe-venmo** v1.2.2
- **@paysafe/paysafe-payments-sdk-common** v1.0.3
- **@paysafe/react-native-paysafe-apple-pay** v0.0.0

### Security

- **CRITICAL: Fixed Dependency Confusion Vulnerability (CVE-TBD)** — Resolved P1 security vulnerability where internal package names could be hijacked via dependency confusion attacks. All SDK packages are now properly scoped under `@paysafe` and published to npm registry, preventing malicious package injection. This addresses the vulnerability reported via bug bounty where attackers could register unscoped package names (`paysafe-card-payments`, `paysafe-google-pay`, etc.) and execute arbitrary code in consuming applications.
