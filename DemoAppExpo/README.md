...
- **`app/`** – Expo Router (file-based routing)
  ...
  - `(tabs)/index.tsx` – Payments hub (SDK setup, Venmo, Google Pay, links to card flows)
...
This app demonstrates `@paysafe/paysafe-venmo` v2.0.2 with the promise-based API (`initializeVenmo`, `tokenizeVenmo`). iOS setup is shown via AppDelegate app switch, `Info.plist`, and Expo Router `app/+native-intent.tsx` (to keep the Venmo screen mounted during app switch). For Expo SDK 53+, the package config plugin can inject AppDelegate wiring on prebuild — see [packages/paysafe-venmo/README.md](../packages/paysafe-venmo/README.md) for Objective‑C, Swift, Expo plugin, and usage details.
