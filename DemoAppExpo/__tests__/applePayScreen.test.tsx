...
const TEST_APPLE_PAY_ACCOUNT_ID = '9284716503';
const TEST_APPLE_PAY_MERCHANT_ID = 'merchant.com.zzzzzzzzzzz';
...
function registerPlatformAndInitSuite(m: ApplePayMocks): void {
  ...
  describe('platform and init', () => {
    ...
    it('on iOS without account env shows init error in status', async () => {
      delete process.env.EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID;
      delete process.env['EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID'];
      ...
      await waitFor(() => {
        expect(mockInit).toHaveBeenCalledWith(
          expect.objectContaining({
            currencyCode: 'USD',
            accountId: TEST_APPLE_PAY_ACCOUNT_ID,
            merchantIdentifier: TEST_APPLE_PAY_MERCHANT_ID,
            countryCode: 'US',
          })
        );
      });
...
describe('ApplePayScreen', () => {
  ...
  beforeEach(() => {
    ...
    Platform.OS = 'ios';
    process.env.EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID = TEST_APPLE_PAY_ACCOUNT_ID;
    process.env['EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID'] = TEST_APPLE_PAY_ACCOUNT_ID;
    process.env.EXPO_PUBLIC_APPLE_PAY_MERCHANT_ID = TEST_APPLE_PAY_MERCHANT_ID;
    process.env['EXPO_PUBLIC_APPLE_PAY_MERCHANT_ID'] = TEST_APPLE_PAY_MERCHANT_ID;
