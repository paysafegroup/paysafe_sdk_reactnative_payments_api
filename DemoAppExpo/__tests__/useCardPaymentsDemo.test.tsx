...
jest.mock('../app/cardPayments/cardPaymentsDemoConfig', () => ({
...
import { useCardPaymentsDemo } from '../app/cardPayments/useCardPaymentsDemo';
...
describe('useCardPaymentsDemo', () => {
  ...
  it('skips native event subscriptions when PaysafeCardPayments module missing', async () => {
    const prev = NativeModules.PaysafeCardPayments;
    delete NativeModules.PaysafeCardPayments;
    const { result } = renderHook(() => useCardPaymentsDemo());
    await waitFor(() => expect(result.current.sdkReady).toBe(true));
    act(() => {
      global.__mockNativeEventEmitterEmit?.('CardPaymentInitialized');
    ...
    expect(result.current.isCardPaymentInitialized).toBe(false);
    NativeModules.PaysafeCardPayments = prev;
  });
});
