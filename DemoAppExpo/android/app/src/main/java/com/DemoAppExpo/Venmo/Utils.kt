...
object VenmoUtils {
  ...
  fun provideVenmoTokenizeOptions(): ReadableMap =
    Arguments.createMap().apply {
      ...
      // merchantRefNum is unused for this demo native Venmo path; SDK still requires the field.
      putString("merchantRefNum", "")
      putMap("billingDetails", createBillingDetails())
      putMap("profile", createProfile())
      putString("accountId", BuildConfig.PAYSAFE_VENMO_ACCOUNT_ID)
  ...
  private fun createVenmoRequest(): WritableMap =
    Arguments.createMap().apply {
      putString("consumerId", "consumerrr@mail.com")
      putString("merchantAccountId", BuildConfig.PAYSAFE_VENMO_ACCOUNT_ID)
      putString("profileId", BuildConfig.PAYSAFE_VENMO_PROFILE_ID)
    }
}
