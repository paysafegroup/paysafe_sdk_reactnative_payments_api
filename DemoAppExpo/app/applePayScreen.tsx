...
function getConfig() {
  ...
  const environment = (process.env.EXPO_PUBLIC_PAYSAFE_ENVIRONMENT?.trim() ?? 'TEST') as
    | 'TEST'
    | 'PROD';
  const accountId = readExpoPublicEnv('EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID');
  const merchantIdentifier = readExpoPublicEnv('EXPO_PUBLIC_APPLE_PAY_MERCHANT_ID');
  ...
  useEffect(() => {
    ...
    async function run(): Promise<void> {
      ...
      if (!hasApplePayAccountConfig(config)) {
        setInitError(
          'Set EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID and EXPO_PUBLIC_APPLE_PAY_MERCHANT_ID in .env (see .env.example).'
        );
      ...
      <SafeAreaView style={styles.safeArea} edges={['bottom']}>
        <ScrollView contentContainerStyle={paymentsStyles.applePayScroll}>
          <Text style={[paymentsStyles.message, styles.messageCentered]}>
            Set EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID and EXPO_PUBLIC_APPLE_PAY_MERCHANT_ID in .env. Sample billing
            address and merchantRefNum from getMerchantReferenceNumber(). Use a dev build and configure Apple Pay in
            Xcode.
          </Text>
          ...
          <View style={paymentsStyles.applePayStatusBox}>
            ...
            <Text style={paymentsStyles.applePayStatusValue}>
              accountId={accountId ?? '— (set EXPO_PUBLIC_PAYSAFE_APPLE_PAY_ACCOUNT_ID)'}{'\n'}
