...
// Mock NativeEventEmitter so tests can fire card-payment events via global.__mockNativeEventEmitterEmit.
const nativeEventTestListeners = new Map();

class MockNativeEventEmitter {
  addListener(event, handler) {
    if (!nativeEventTestListeners.has(event)) {
      nativeEventTestListeners.set(event, new Set());
    }
    nativeEventTestListeners.get(event).add(handler);
    return { remove: () => nativeEventTestListeners.get(event)?.delete(handler) };
  }
}

global.__mockNativeEventEmitterEmit = (event, ...args) => {
  const subs = nativeEventTestListeners.get(event);
  if (!subs) {
    return;
  }
  subs.forEach((h) => {
...
global.__mockNativeEventEmitterClear = () => nativeEventTestListeners.clear();
