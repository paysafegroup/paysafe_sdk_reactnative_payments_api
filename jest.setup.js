...
jest.mock('react-native', () => {
  ...
  global.__mockNativeEventEmitterClear = () => {
    ...
    Object.keys(deviceEventEmitterListeners).forEach((k) => delete deviceEventEmitterListeners[k]);
  };

  const deviceEventEmitterListeners = {};
  const DeviceEventEmitter = {
    addListener: jest.fn((eventName, callback) => {
      deviceEventEmitterListeners[eventName] = deviceEventEmitterListeners[eventName] || [];
      deviceEventEmitterListeners[eventName].push(callback);
      return {
        remove: jest.fn(() => {
          const arr = deviceEventEmitterListeners[eventName];
          if (arr) {
            const i = arr.indexOf(callback);
            if (i >= 0) arr.splice(i, 1);
          }
    ...
    emit: jest.fn((eventName, ...args) => {
      const callbacks = deviceEventEmitterListeners[eventName] || [];
      callbacks.forEach((cb) => cb(...args));
    }),
  };
  ...
  return {
    ...
    TurboModuleRegistry: {
      get: jest.fn((name) => {
        ...
        if (name === 'RNPaysafeApplePay') {
          return {
            initializeContext: jest.fn().mockResolvedValue(undefined),
            resetContext: jest.fn().mockResolvedValue(undefined),
            tokenize: jest.fn().mockResolvedValue({ token: 'apple-pay-token-12345', isSuccess: true }),
            isApplePayAvailable: jest.fn().mockResolvedValue({
              isAvailable: true,
