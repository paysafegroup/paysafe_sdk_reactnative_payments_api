...
import NativeRNPaysafeApplePay from './NativeRNPaysafeApplePay';
import type {
...
export async function initializeApplePayContext(
  ...
  await NativeRNPaysafeApplePay.initializeContext({
...
export async function resetApplePayContext(): Promise<void> {
  ...
  await NativeRNPaysafeApplePay.resetContext();
}
...
export async function tokenize(options: PaysafeApplePayTokenizeOptions): Promise<PaysafeApplePayPaymentResult> {
  ...
  try {
    ...
    const result = (await NativeRNPaysafeApplePay.tokenize(payload)) as {
      token?: string;
      isSuccess?: boolean;
  ...
  try {
    const nativeReq =
      request?.supportedNetworks && request.supportedNetworks.length > 0
        ...
        : {};
    const result = (await NativeRNPaysafeApplePay.isApplePayAvailable(nativeReq)) as {
      isAvailable?: boolean;
