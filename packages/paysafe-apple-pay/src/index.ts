...
import { getNativeModule } from './NativeRNPaysafeApplePay';
import type {
...
export async function initializeApplePayContext(
  ...
  await getNativeModule().initializeContext({
...
export async function resetApplePayContext(): Promise<void> {
  ...
  await getNativeModule().resetContext();
}
...
export async function tokenize(options: PaysafeApplePayTokenizeOptions): Promise<PaysafeApplePayPaymentResult> {
  ...
  try {
    ...
    const result = (await getNativeModule().tokenize(payload)) as {
      token?: string;
      isSuccess?: boolean;
  ...
  try {
    const nativeReq =
      request?.supportedNetworks && request.supportedNetworks.length > 0
        ...
        : {};
    const result = (await getNativeModule().isApplePayAvailable(nativeReq)) as {
      isAvailable?: boolean;
