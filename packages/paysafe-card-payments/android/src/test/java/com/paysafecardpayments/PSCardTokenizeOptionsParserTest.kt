...
class PSCardTokenizeOptionsParserTest {
  ...
  fun `happy path with all nested fields`() {
    ...
    assertEquals(2, result.profile?.identityDocuments?.size)
    assertEquals("ID123456789", result.profile?.identityDocuments?.get(0)?.documentNumber)
    assertEquals("PASS987654321", result.profile?.identityDocuments?.get(1)?.documentNumber)
  ...
  @Test
  fun `buildIdentityDocuments returns null when array is null`() {
    // given
    val map = mock<ReadableMap>()
    whenever(map.getInt(AMOUNT)).thenReturn(100)
    whenever(map.getString(CURRENCY_CODE)).thenReturn("USD")
    whenever(map.getString(MERCHANT_REF_NUM)).thenReturn("ref123")
    whenever(map.getString(ACCOUNT_ID)).thenReturn("acc123")
    ...
    // then
    assertEquals(null, result.profile?.identityDocuments)
  }

  @Test
  fun `buildIdentityDocuments returns empty list when array is empty`() {
    // given
    val map = mock<ReadableMap>()
    whenever(map.getInt(AMOUNT)).thenReturn(100)
    whenever(map.getString(CURRENCY_CODE)).thenReturn("USD")
    ...
    whenever(map.getString(ACCOUNT_ID)).thenReturn("acc123")
    val profileMap = mock<ReadableMap>()
    val emptyArray = mock<ReadableArray>()
    whenever(emptyArray.toArrayList()).thenReturn(arrayListOf())
    whenever(profileMap.getArray(IDENTITY_DOCUMENTS)).thenReturn(emptyArray)
    whenever(map.getMap(PROFILE)).thenReturn(profileMap)
    ...
    // then
    assertEquals(emptyList<IdentityDocument>(), result.profile?.identityDocuments)
  }
  ...
  private fun mockProfile(): ReadableMap {
    val profileMap = mock<ReadableMap>()
    val dateOfBirthMap = mockDateOfBirth()
    val identityDocumentsArray = mockIdentityDocuments()
  ...
  private fun mockDateOfBirth(): ReadableMap {
  ...
  private fun mockIdentityDocuments(): ReadableArray {
    val identityDocumentsArray = mock<ReadableArray>()
    val documentMap1 = mapOf("documentNumber" to "ID123456789")
    val documentMap2 = mapOf("documentNumber" to "PASS987654321")
    // doAnswer + raw Answer: ReadableArray.toArrayList() is ArrayList<Any> vs ArrayList<Any?>
    // depending on RN/Kotlin; thenReturn() fails one CI or the other.
    Mockito.doAnswer(
      Answer {
