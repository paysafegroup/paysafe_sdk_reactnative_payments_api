...
class PaysafeCardPaymentsModuleTest {
  ...
  fun `test getMerchantReferenceNumber returns correct value`() {
    // given
    every { PaysafeSDK.getMerchantReferenceNumber() } returns MERCHANT_REFERENCE_NUMBER
    paysafeCardPaymentsModule = PaysafeCardPaymentsModule(mockReactContext, psCardTokenizeOptionsParser)

    // when
    val result = paysafeCardPaymentsModule.getMerchantReferenceNumber()

    // then
    verify(exactly = 1) {
      PaysafeSDK.getMerchantReferenceNumber()
    }
    assertEquals(MERCHANT_REFERENCE_NUMBER, result)
  }
