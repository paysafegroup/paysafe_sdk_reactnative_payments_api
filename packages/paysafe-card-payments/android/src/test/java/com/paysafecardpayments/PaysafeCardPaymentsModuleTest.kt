...
class PaysafeCardPaymentsModuleTest {
  ...
  fun `getMerchantReferenceNumber resolves promise with correct value`() {
    ...
    every { PaysafeCardPaymentsModule.getMerchantReferenceNumber() } returns expectedRef

    // when
    paysafeCardPaymentsModule.getMerchantReferenceNumber(promise)

    // then
    verify(exactly = 1) { PaysafeCardPaymentsModule.getMerchantReferenceNumber() }
  ...
  fun `getMerchantReferenceNumber rejects promise when exception is thrown`() {
    ...
    mockkObject(PaysafeCardPaymentsModule.Companion)
    every { PaysafeCardPaymentsModule.getMerchantReferenceNumber() } throws exception
    ...
    // then
    verify(exactly = 1) {
      PaysafeCardPaymentsModule.getMerchantReferenceNumber()
    }
    verify(exactly = 1) {
