...
class PSCardHolderNameViewManagerTest {

  private lateinit var reactAppContext: ReactApplicationContext
  private lateinit var viewManager: PSCardholderNameViewManager
  private lateinit var mockView: PSCardholderNameWrapperView
  ...
  @Test
  fun `createWrapperView returns PSCardholderNameWrapperView when activity is not null`() {
    // given
    ...
    viewManager = PSCardholderNameViewManager(reactAppContext) { mockActivity ->
  ...
  fun `createWrapperView throws IllegalStateException when activity is null`() {
    ...
    viewManager = PSCardholderNameViewManager(reactAppContext)

    // when then
    val exception = assertThrows(IllegalStateException::class.java) {
      viewManager.createWrapperView(themedReactContext)
    }
    assertTrue(exception.message!!.contains("Activity is null when creating PSCardholderNameWrapperView"))
  }

  @Test
  fun `getName returns correct view name`() {
    // given
    viewManager = PSCardholderNameViewManager(reactAppContext) { mockView }

    // when then
    val name = viewManager.name
    assertTrue(name == "PSCardholderNameView")
  }
}
