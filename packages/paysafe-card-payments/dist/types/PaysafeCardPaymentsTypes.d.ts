/**
 * Options for card payment tokenization.
 * Mirrors Kotlin: PaymentHandleRequest / ThreeDS models
 */
export interface CardPaymentsTokenizeOptions {
    ...
    /**
     * 3DS simulator mode. Defaults to EXTERNAL (matches Android native parser).
     */
    simulator?: 'EXTERNAL' | 'INTERNAL';
    /** Rendering type for the card entry view */
    renderType?: 'NATIVE' | 'HTML' | 'BOTH';
}
/**
...
export interface ThreeDS {
    ...
    authenticationPurpose?: 'PAYMENT_TRANSACTION' | 'RECURRING_TRANSACTION' | 'INSTALMENT_TRANSACTION' | 'ADD_CARD' | 'MAINTAIN_CARD' | 'EMV_TOKEN_VERIFICATION';
...
export interface BillingCycle {
    endDate?: string;
    /** Recurring billing frequency (minor units / count per backend contract; iOS/Android use integer). */
    frequency?: number;
}
...
export interface UserAccountDetails {
    ...
    changedDate?: string;
    changedRange?: 'DURING_TRANSACTION' | 'LESS_THAN_THIRTY_DAYS' | 'THIRTY_TO_SIXTY_DAYS' | 'MORE_THAN_SIXTY_DAYS';
...
export interface ShippingDetailsUsage {
    ...
    /** @deprecated Prefer `initialUsageDate` (matches Android/iOS native keys). */
    creationDate?: string;
    /** Length of time since first use of this shipping address (native key). */
    initialUsageDate?: string;
    initialUsageRange?: 'CURRENT_TRANSACTION' | 'LESS_THAN_THIRTY_DAYS' | 'THIRTY_TO_SIXTY_DAYS' | 'MORE_THAN_SIXTY_DAYS';
}
...
export interface ShippingDetails {
    shipMethod?: 'NEXT_DAY_OR_OVERNIGHT' | 'TWO_DAY_SERVICE' | 'LOWEST_COST' | 'OTHER';
    street?: string;
    street2?: string;
    city?: string;
    state?: string;
...
//# sourceMappingURL=PaysafeCardPaymentsTypes.d.ts.map