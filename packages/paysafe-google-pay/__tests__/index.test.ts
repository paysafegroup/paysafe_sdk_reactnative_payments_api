...
function describeSetupPaysafeSdk() {
  describe('setupPaysafeSdk function', () => {
    ...
    it('defaults environment to TEST when omitted', () => {
      const mockSetup = jest.fn().mockReturnValue('ok');
      jest.resetModules();
      jest.doMock(REACT_NATIVE, () => ({
        NativeModules: {
          PaysafeGooglePay: { setupPaysafeSdk: mockSetup },
        },
        Platform: { OS: 'android' },
      }));
      const { setupPaysafeSdk } = require('../src');
      setupPaysafeSdk('api-key-only');
