...
class PaysafeGooglePayModuleTest {
  ...
  fun `getMerchantReferenceNumber resolves promise with correct value`() {
    ...
    mockPaysafeGooglePayModule = PaysafeGooglePayModule(mockReactContext, psGooglePayTokenizeOptionsParser)

    mockkObject(PaysafeGooglePayModule.Companion)
    every { PaysafeGooglePayModule.Companion.getMerchantReferenceNumber() } returns expectedRef

    // when
    mockPaysafeGooglePayModule.getMerchantReferenceNumber(promise)

    // then
    verify(exactly = 1) { PaysafeGooglePayModule.Companion.getMerchantReferenceNumber() }
  ...
  fun `getMerchantReferenceNumber rejects promise when exception is thrown`() {
    ...
    every { PaysafeGooglePayModule.Companion.getMerchantReferenceNumber() } throws exception
    ...
    // then
    verify(exactly = 1) {
      PaysafeGooglePayModule.Companion.getMerchantReferenceNumber()
    }
    verify(exactly = 1) {
