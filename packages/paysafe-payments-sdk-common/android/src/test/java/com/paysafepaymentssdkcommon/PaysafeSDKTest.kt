...
class PaysafeSDKTest {
  ...
  fun `test setup calls native setup method`() {
    ...
    // when
    paysafeSDK.setup(apiKey, environment, promise)
  ...
  fun `test setup rejects promise for invalid environment`() {
    val promise = mockk<Promise>(relaxed = true)
    paysafeSDK.setup("apiKey", "INVALID_ENV", promise)
    // Match reject(code, message, throwable); matchers are MockKMatcherScope extensions inside verify.
    verify(exactly = 1) {
