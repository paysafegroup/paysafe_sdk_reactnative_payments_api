...
class PaysafeVenmoModule(
  ...
  fun initialize(currencyCode: String, accountId: String) {
    Companion.initializeFromReactNative(currencyCode, accountId)
  }
  ...
  companion object {
    ...
    /**
     * Initializes Venmo from a React Native @ReactMethod. Attaches a headless [VenmoInitFragment]
     * so the Paysafe SDK can register its ActivityResultLauncher before STARTED.
     */
    fun initializeFromReactNative(currencyCode: String, accountId: String) {
      val reactApplicationContext = SingletonVenmoContext.getReactApplicationContext()

      UiThreadUtil.runOnUiThread {
        if (venmoContext != null) {
          reactApplicationContext?.let { sendEvent(it, VENMO_INITIALIZATION_SUCCESSFUL) }
    ...
    fun tokenize(
      ...
      // Prefer the foreground activity (matches native sample: tokenize(requireContext(), …)).
      // Application context can behave differently for intents / theming when launching checkout.
      val tokenizeContext =
