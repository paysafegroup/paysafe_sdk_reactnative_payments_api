...
class PSVenmoTokenizeOptionsParserTest {
  ...
  fun `should parse all fields correctly`() {
    ...
    assertNotNull(result.profile!!.identityDocuments)
    assertEquals(1, result.profile!!.identityDocuments!!.size)
    assertEquals("ID123456789", result.profile!!.identityDocuments!![0].documentNumber)
  ...
  @Test
  fun `should parse INTERNAL simulator when present`() {
    val map = mock<ReadableMap>()
    whenever(map.getInt(AMOUNT)).thenReturn(1000)
    whenever(map.getString(CURRENCY_CODE)).thenReturn("USD")
    whenever(map.getString(MERCHANT_REF_NUM)).thenReturn("ref123")
    whenever(map.getString(ACCOUNT_ID)).thenReturn("acc456")
    whenever(map.getString(SIMULATOR)).thenReturn("INTERNAL")

    val result = parser.fromReadableMap(map)

    assertEquals(SimulatorType.INTERNAL, result.simulator)
  }

  @Test
  fun `should map venmoRequest null optional ids to empty strings`() {
    val map = mock<ReadableMap>()
    val venmoMap = mock<ReadableMap>()
    whenever(map.getInt(AMOUNT)).thenReturn(1000)
    whenever(map.getString(CURRENCY_CODE)).thenReturn("USD")
    ...
    whenever(map.getString(ACCOUNT_ID)).thenReturn("acc456")
    whenever(map.getMap(VENMO_REQUEST)).thenReturn(venmoMap)
    whenever(venmoMap.getString(CONSUMER_ID)).thenReturn("cons789")
    whenever(venmoMap.getString(MERCHANT_ACCOUNT_ID)).thenReturn(null)
    whenever(venmoMap.getString(PROFILE_ID)).thenReturn(null)

    val result = parser.fromReadableMap(map)

    assertEquals("", result.venmoRequest!!.merchantAccountId)
    assertEquals("", result.venmoRequest!!.profileId)
  }

  @Test(expected = IllegalArgumentException::class)
  fun `should throw when billing zip is missing`() {
    val map = mock<ReadableMap>()
    val billingMap = mock<ReadableMap>()
    whenever(map.getInt(AMOUNT)).thenReturn(1000)
    whenever(map.getString(CURRENCY_CODE)).thenReturn("USD")
    ...
    whenever(billingMap.getString(ZIP)).thenReturn(null)

    parser.fromReadableMap(map)
  }

  @Test(expected = IllegalArgumentException::class)
  fun `should throw when merchantDescriptor dynamicDescriptor is missing`() {
    val map = mock<ReadableMap>()
    val mdMap = mock<ReadableMap>()
    whenever(map.getInt(AMOUNT)).thenReturn(1000)
    whenever(map.getString(CURRENCY_CODE)).thenReturn("USD")
    whenever(map.getString(MERCHANT_REF_NUM)).thenReturn("ref123")
    whenever(map.getString(ACCOUNT_ID)).thenReturn("acc456")
    whenever(map.getMap(MERCHANT_DESCRIPTOR)).thenReturn(mdMap)
    whenever(mdMap.getString(DYNAMIC_DESCRIPTOR)).thenReturn(null)

    parser.fromReadableMap(map)
  }

  @Test
  fun `profile dateOfBirth omits day when key absent`() {
    val map = mock<ReadableMap>()
    val profileMap = mock<ReadableMap>()
    val dobMap = mock<ReadableMap>()
    whenever(map.getInt(AMOUNT)).thenReturn(1000)
    whenever(map.getString(CURRENCY_CODE)).thenReturn("USD")
    whenever(map.getString(MERCHANT_REF_NUM)).thenReturn("ref123")
    whenever(map.getString(ACCOUNT_ID)).thenReturn("acc456")
    whenever(map.getMap(PROFILE)).thenReturn(profileMap)
    whenever(profileMap.getString(FIRST_NAME)).thenReturn("A")
    whenever(profileMap.getString(LAST_NAME)).thenReturn("B")
    whenever(profileMap.getString(EMAIL)).thenReturn("a@b.com")
    whenever(profileMap.getMap(DATE_OF_BIRTH)).thenReturn(dobMap)
    whenever(profileMap.getArray(IDENTITY_DOCUMENTS)).thenReturn(null)
    whenever(dobMap.hasKey(DAY)).thenReturn(false)
    whenever(dobMap.hasKey(MONTH)).thenReturn(true)
    whenever(dobMap.isNull(MONTH)).thenReturn(false)
    whenever(dobMap.getInt(MONTH)).thenReturn(5)
    whenever(dobMap.hasKey(YEAR)).thenReturn(true)
    whenever(dobMap.isNull(YEAR)).thenReturn(false)
    whenever(dobMap.getInt(YEAR)).thenReturn(2000)

    val result = parser.fromReadableMap(map)

    assertEquals(null, result.profile!!.dateOfBirth!!.day)
    assertEquals(5, result.profile!!.dateOfBirth!!.month)
    assertEquals(2000, result.profile!!.dateOfBirth!!.year)
  }
  ...
  private fun mockProfileMap(profileMap: ReadableMap) {
    ...
    val identityDocumentsArray = mock<ReadableArray>()
    ...
    whenever(profileMap.getArray(IDENTITY_DOCUMENTS)).thenReturn(identityDocumentsArray)
    mockDateOfBirthMap(dateOfBirthMap)
    mockIdentityDocumentsArray(identityDocumentsArray)
  }

  private fun mockDateOfBirthMap(dateOfBirthMap: ReadableMap) {
  ...
  private fun mockIdentityDocumentsArray(identityDocumentsArray: ReadableArray) {
    val documentMap = hashMapOf<String, Any>(
      DOCUMENT_NUMBER to "ID123456789"
    )
    Mockito.doAnswer(
      Answer {
        java.util.ArrayList<Any?>().apply { add(documentMap) }
      }
    ).`when`(identityDocumentsArray).toArrayList()
  }
