...
class PaysafeVenmoModuleTest {
  ...
  @Test
  fun `initializeFromReactNative should log and do nothing when react context is null`() {
    // given
  ...
  @Test
  fun `initializeFromReactNative should emit success when venmo context is already initialized`() {
    // given
    val mockActivity = Robolectric.buildActivity(FragmentActivity::class.java).setup().get()
    val mockEmitter = mockk<DeviceEventManagerModule.RCTDeviceEventEmitter>(relaxed = true)
    paysafeVenmoModule = PaysafeVenmoModule(mockReactContext, psVenmoTokenizeOptionsParser)
